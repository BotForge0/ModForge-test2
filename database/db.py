...
class Database:
    ...
    # Hilfsfunktionen
    def _trim_locks(self, locks_dict: Dict[int, asyncio.Lock], max_size: int = 100):
    ...
    async def set_config(self, guild_id: int, cfg: dict):
        cfg = dict(cfg)
        cfg["_id"] = guild_id
        try:
    ...
    async def update_module(self, guild_id: int, module: str, key: str, value: Any):
    ...
    async def aset_config(self, guild_id: int, cfg: dict):
        await self.set_config(guild_id, cfg)

    async def aupdate_module(self, guild_id: int, module: str, key: str, value: Any):
    ...
    async def set_whitelist(self, guild_id: int, whitelist: dict):
    ...
    async def add_whitelist(self, guild_id: int, category: str, entry_id: int):
    ...
    async def remove_whitelist(self, guild_id: int, category: str, entry_id: int):
        data = await self.aget_whitelist(guild_id)
        data = dict(data)
        if category in data:
    ...
    async def aadd_whitelist(self, guild_id: int, category: str, entry_id: int):
        await self.add_whitelist(guild_id, category, entry_id)

    async def aremove_whitelist(self, guild_id: int, category: str, entry_id: int):
    ...
    async def adeactivate_mute(self, guild_id: int, user_id: int):
        try:
    ...
    async def aadd_tempaction(self, action: str, guild_id: int, user_id: int,
                              end_time: datetime.datetime, reason: str, mod_id: int):
        try:
    ...
    async def adeactivate_tempaction(self, _id: Any):
        try:
    ...
    async def amark_message_deleted(self, guild_id: int, message_id: int):
        try:
