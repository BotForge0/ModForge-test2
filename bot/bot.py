...
class Tracker:
    ...
    @staticmethod
    def clean_old(dq: deque, window: float):
        now = time.time()
...
class CaptchaModal(discord.ui.Modal, title="Verifizierung"):
    ...
    def __init__(self, correct_code: str, bot_ref, add_roles: List[int], remove_roles: List[int]):
        super().__init__()
...
class CaptchaEntryView(discord.ui.View):
    def __init__(self, code: str, bot_ref, add_roles: List[int], remove_roles: List[int]):
...
class AppealReasonModal(discord.ui.Modal):
    def __init__(self, action: str, appeal_data: dict, bot_ref):
...
class AppealActionView(discord.ui.View):
    def __init__(self, bot_ref=None, appeal_data: Optional[dict] = None):
        super().__init__(timeout=None)
        self.bot_ref = bot_ref
    ...
    async def unban_reason(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.user.guild_permissions.ban_members:
            await interaction.response.send_message("Keine Berechtigung.", ephemeral=True)
            return
        data = self._get_appeal_data(interaction)
...
class ModForge(commands.Bot):
    ...
    async def log_action(self, guild, title, description, color=COLOR_INFO, fields=None, user=None, module="default"):
        if not guild:
            return
    ...
    async def _check_warn_thresholds(self, member, warn_count: int):
    ...
    async def tempaction_loop(self):
        try:
    ...
    @tasks.loop(count=1)
    async def restore_persistent_mutes(self):
...
async def _submit_appeal(user: discord.User, session: dict):
...
async def _activate_lockdown(guild: discord.Guild):
...
async def _deactivate_lockdown(guild: discord.Guild):
    for channel in guild.text_channels:
        try:
...
async def _auto_deactivate_lockdown(guild: discord.Guild, delay: int):
...
async def _notify_owner_nuke(guild: discord.Guild, executor: Optional[discord.Member],
                             executor_id: int, action: str, punishment: str):
    owner = guild.owner
    if not owner:
        try:
...
async def _nuke_check(guild: discord.Guild, executor_id: int, action: str):
    cfg = bot.db.get_config(guild.id)
    nuke_cfg = cfg.get("anti_nuke", {})
...
@has_mod_perms()
async def prefix_unmute(ctx: commands.Context, member: discord.Member):
    try:
...
@has_mod_perms()
async def prefix_clear(ctx: commands.Context, amount: int = 10):
    try:
...
@has_mod_perms()
async def prefix_slowmode(ctx: commands.Context, seconds: int = 0):
    try:
...
@has_mod_perms()
async def prefix_lock(ctx: commands.Context):
    try:
...
@has_mod_perms()
async def prefix_unlock(ctx: commands.Context):
    try:
...
@has_mod_perms()
async def prefix_warnings(ctx: commands.Context, member: discord.Member):
...
@has_mod_perms()
async def prefix_clearwarnings(ctx: commands.Context, member: discord.Member):
...
@has_mod_perms()
async def prefix_unban(ctx: commands.Context, user_id: int):
    try:
...
@has_mod_perms()
async def prefix_nick(ctx: commands.Context, member: discord.Member, *, nickname: str = None):
    try:
...
async def cmd_banappeal(ctx: commands.Context, user_id: int, *,
                        ban_reason: str = "Kein Grund angegeben"):
    cfg = bot.db.get_config(ctx.guild.id)
...
async def setup_verify(interaction: discord.Interaction, channel: discord.TextChannel,
                       mode: str, difficulty: str = "medium"):
...
async def slash_tempban(interaction: discord.Interaction, member: discord.Member,
                        duration: str, reason: str = "Kein Grund"):
    ok, why = can_moderate(interaction.user, member, interaction.guild.me)
    if not ok:
        await interaction.response.send_message(
            embed=create_embed(f"{E.FAIL} Aktion blockiert", why, COLOR_DANGER), ephemeral=True)
...
@app_commands.default_permissions(manage_messages=True)
async def slash_clearwarning(interaction: discord.Interaction, member: discord.Member, index: int):
...
@app_commands.default_permissions(administrator=True)
async def security_view(interaction: discord.Interaction):
    cfg = bot.db.get_config(interaction.guild.id)
...
@app_commands.default_permissions(administrator=True)
async def security_toggle(interaction: discord.Interaction, module: str):
    cfg = bot.db.get_config(interaction.guild.id)
...
@app_commands.default_permissions(administrator=True)
async def security_set(interaction: discord.Interaction, module: str, setting: str, value: str):
    parsed: Any = value
...
@app_commands.default_permissions(administrator=True)
async def security_reset(interaction: discord.Interaction, module: str):
...
async def whitelist_add(interaction: discord.Interaction,
                        category: app_commands.Choice[str], entry_id: str):
...
async def whitelist_bulk_add(interaction: discord.Interaction,
                             category: app_commands.Choice[str], ids: str):
...
@app_commands.default_permissions(manage_guild=True)
async def automod_regex_add(interaction: discord.Interaction, pattern: str):
    try:
        re.compile(pattern)
...
@app_commands.default_permissions(manage_guild=True)
async def automod_regex_remove(interaction: discord.Interaction, index: int):
    cfg = bot.db.get_config(interaction.guild.id)
    rules = cfg.get("automod", {}).get("regex_rules", [])
...
@app_commands.default_permissions(manage_guild=True)
async def automod_regex_list(interaction: discord.Interaction):
    cfg = bot.db.get_config(interaction.guild.id)
    rules = cfg.get("automod", {}).get("regex_rules", [])
    if not rules:
...
@app_commands.default_permissions(manage_guild=True)
async def automod_domain_allow(interaction: discord.Interaction, domain: str):
    cfg = bot.db.get_config(interaction.guild.id)
...
@app_commands.default_permissions(administrator=True)
async def slash_setarchive(interaction: discord.Interaction, enabled: bool):
    cfg = bot.db.get_config(interaction.guild.id)
...
@app_commands.default_permissions(administrator=True)
async def slash_audit_perms(interaction: discord.Interaction):
...
@app_commands.default_permissions(administrator=True)
async def slash_logview(interaction: discord.Interaction):
    cfg = bot.db.get_config(interaction.guild.id)
...
@app_commands.default_permissions(administrator=True)
async def slash_logmodules(interaction: discord.Interaction):
...
@app_commands.default_permissions(administrator=True)
async def slash_panic(interaction: discord.Interaction):
...
@app_commands.default_permissions(administrator=True)
async def slash_massunban(interaction: discord.Interaction):
    await interaction.response.defer()
...
@app_commands.default_permissions(administrator=True)
async def slash_massrole_remove(interaction: discord.Interaction, role: discord.Role):
    await interaction.response.defer()
    count = 0
...
@app_commands.default_permissions(administrator=True)
async def slash_warnsetup(interaction: discord.Interaction):
...
@app_commands.default_permissions(administrator=True)
async def slash_logban(interaction: discord.Interaction, channel: discord.TextChannel):
    cfg = bot.db.get_config(interaction.guild.id)
